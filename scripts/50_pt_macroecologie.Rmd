...
# Calcul des indicateurs de diversité au point
...
```{r}
pt_effectif <- aspe %>% 
  filter(str_detect(type_peche, pattern = "partielle|ambiances|complète"),
         effectif > 0) %>% 
...
```{r}
pt_effectif_matrice <- pt_effectif %>% 
  pivot_wider(id_cols = code_point,
              names_from = code_espece,
              values_from = effectif,
              values_fill = 0) %>% 
  column_to_rownames(var = "code_point")


# Calcul des indices
pt_indices <- pt_effectif_matrice %>%
  transmute(
    richesse = specnumber(.),
    shannon = diversity(.),
  ...
  rownames_to_column(var = "code_point")
```
